...
export const createVariable = async (variableData) => {
  if (!variableData.name || !variableData.value) {
    throw new Error('Name and value are required for variables')
  }
 ...
 * }>} Updated variable
 */
export const updateVariable = async (id, variableData) => {
  if (!variableData.name || !variableData.value) {
    throw new Error('Name and value are required for variables')
  }

  try {
    const response = await api.put(`/variables/${id}`, variableData)
    return response.data
  } catch (error) {
    throw error
  }
